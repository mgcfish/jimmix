...
public class CommandLineParser {
    ...
    private static void createAction(ProxyType type) throws Exception {
        ...
        if (action.equals("invoke")) {
            ...
            // mlet
        } else if (action.equals("mlet")) {
            if (args.length < 4) {
                printParsingError("Too few arguments.");
            }
            if (args.length > 5) {
                printParsingError("Too many arguments.");
            }
            String hostingUrl = args[1];
            String jarName = args[2];
            String className = args[3];
            boolean internalHttpServer = true;
            if (args.length == 5) {
                if (args[4].equalsIgnoreCase("false")) {
                    internalHttpServer = false;
                } else if (args[4].equalsIgnoreCase("true")) {
                    internalHttpServer = true;
                } else {
                    printParsingError("Unknown value: " + args[4]);
                }
            }

            parsedCommand.put("action", new MLetAction((String) parsedCommand.get("url"), type, hostingUrl, jarName, className, internalHttpServer));
